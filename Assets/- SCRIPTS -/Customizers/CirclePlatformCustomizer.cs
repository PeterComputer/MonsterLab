...
[ExecuteInEditMode]
public class CirclePlatformCustomizer : MonoBehaviour
{
    ...
    private void setName(ColorEnum newColor)
    {
        if (colorIndex == -1)
        {
            // Find all CirclePlatformCustomizers in the scene (including inactives)
            CirclePlatformCustomizer[] allPlatforms = FindObjectsByType<CirclePlatformCustomizer>(FindObjectsInactive.Include, FindObjectsSortMode.None);

            // Creates a list and populates it with all the platforms that already exist with that color
            List<CirclePlatformCustomizer> coloredPlatforms = new List<CirclePlatformCustomizer>(50);
            ...
            foreach (CirclePlatformCustomizer platform in allPlatforms)
            {
        ...
        gameObject.name = platformColor.ToString() + " Circular Interactive Platform " + (colorIndex + 1);
    ...
    public void Awake()
    {
        ...
        // If interactible area is null, go get it and save it
        if (interactibleArea == null)
        {
            interactibleArea = GetComponentInChildren<InteractibleArea>();
...
#if UNITY_EDITOR
[CustomEditor(typeof(CirclePlatformCustomizer))]
public class CirclePlatformEditor : Editor
{

    public override void OnInspectorGUI()
    {
        var customizer = (CirclePlatformCustomizer)target;
